...
- [x] Integrate AASIST audio classifier on extracted audio track — `app/models/audio_deepfake.py` loads the real clovaai/aasist ASVspoof2019 LA checkpoint (plain AASIST sinc-conv front end, not yet the XLSR-53 front end CLAUDE.md specifies — see that file's docstring for the flagged substitution and why). Verified against real inference in `backend/tests/test_audio_deepfake.py`. Wired into both the batch worker and the live `/ws` stream.
- [x] Integrate SyncNet lip-sync scorer as a third modality — `app/models/lipsync.py` (real `syncnet_v2.model` checkpoint, `app/pipeline/face_track.py` for continuous 25fps mouth-crop tracking) + `workers/lipsync_worker.py`, enqueued from `scan.py`, registered in `workers/main.py`, weighted into `fusion/scoring.py`. Verified against real inference in `backend/tests/test_lipsync.py`.
- [ ] Build the fusion head (even a simple logistic regression trained on a small labeled set beats a fixed weighted average) — still the interim equal-weighted average per `fusion/scoring.py`'s `equal_weighted_average_interim_pending_stacking_head` method tag.
